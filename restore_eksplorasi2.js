...
async function extract() {
                ...
                if (entry.type === 'PLANNER_RESPONSE' && entry.tool_calls) {
                    for (const call of entry.tool_calls) {
                        if (call.name === 'write_to_file' && call.args && call.args.TargetFile && call.args.TargetFile.includes('eksplorasi.js')) {
                            if (call.args.CodeContent && call.args.CodeContent.length > 5000 && !call.args.CodeContent.includes('require(\\'fs\\')')) {
    ...
    if (lastFullWrite) {
        ...
        console.log("Restored eksplorasi.js! Length: " + lastFullWrite.length);
    } else {
        console.log("Could not find valid write_to_file for eksplorasi.js");
    }
}
extract();
