document.addEventListener('DOMContentLoaded', async () => {
    ...
    let saldoAwal = 0;
    let daftarBelanjaan = [];
    let sisaWaktu = 120; // Default 2 Menit
    ...
    if (btnRaksasaRak) {
        btnRaksasaRak.addEventListener('click', () => { modalRak.style.display = 'flex'; });
    }
    ...
    if (btnMulaiBermain) {
        btnMulaiBermain.addEventListener('click', () => {
            ...
            // Sembunyikan panel pra-kuis dan munculkan area game
    ...
    btnBayar.addEventListener('click', () => {
        ...
        for (let i = 0; i < currentItem.jumlah; i++) {
            ...
            // Efek Partikel Komet (Semburan Komet Putih Bercahaya)
            const baseDelay = i * 200;
            const numParticles = 3; // Mengirim 3 partikel per item
            for (let p = 0; p < numParticles; p++) {
                setTimeout(() => {
                    ...
                    const anim = particle.animate([
                        ...
                        duration: 550, // Durasi seimbang (tidak terlalu cepat/lambat)
                ...
                }, baseDelay + (p * 75)); // Jeda 75ms antar komet
            }
        }
    ...
    function akhiriGame() {
        ...
        // Langsung scroll ke arah laporan evaluasi agar langsung nampak tabelnya
        setTimeout(() => {
    ...
    btnSimpanEvaluasi.addEventListener('click', async () => {
        ...
        try {
            ...
            // Tunggu maksimal 2 detik agar tidak macet
            await Promise.race([
                Promise.all([firestorePromise, resultPromise]),
                new Promise(resolve => setTimeout(resolve, 2000))
            ]);

            // Tampilkan popup persetujuan guru
            Swal.fire({
                ...
                didOpen: () => {
                    ...
                    if (typeof db !== 'undefined' && docRefId) {
                        // Dengarkan persetujuan guru secara real-time
                        db.collection('perencanaan').doc(docRefId).onSnapshot((doc) => {
                            if (doc.exists) {
                                ...
                                if (data.izinLanjut === true) {
                                    ...
                                    // Ubah judul dialog menjadi Berhasil/Disetujui
                                    ...
                                    // Mainkan efek suara success jika ada
                        ...
                        // Fallback jika tidak ada internet/koneksi Firestore
                        confirmBtn.disabled = false;
                        confirmBtn.style.opacity = '1';
                        confirmBtn.style.cursor = 'pointer';
                        confirmBtn.textContent = 'Lanjut Evaluasi 🚀';
